...
std::pair<Status, Window> validate_and_configure_window(ITensorInfo *input, ITensorInfo *output, const Size2D &kernel_dims, const PadStrideInfo &conv_info, bool has_bias, const Size2D &dilation)
{
    ...
    auto_init_if_empty(*output, input->clone()->set_tensor_shape(output_shape));        //会复制input的datatype
        ...
        for(int y = top_left_y; y < y_e; y += dilation_y)
        {
            ...
            else
            {
                  for(int x = top_left_x; x < x_e; x += dilation_x, ++out_ptr)
                {
                    ...
                    {
                        *(out_ptr + 0 * kernel_size2) = *(reinterpret_cast<const int8_t *>(in_ptr + ((d + 0) * input_stride_z + y * input_stride_y + x * input_stride_x)));
                        *(out_ptr + 1 * kernel_size2) = *(reinterpret_cast<const int8_t *>(in_ptr + ((d + 1) * input_stride_z + y * input_stride_y + x * input_stride_x)));
                        *(out_ptr + 2 * kernel_size2) = *(reinterpret_cast<const int8_t *>(in_ptr + ((d + 2) * input_stride_z + y * input_stride_y + x * input_stride_x)));
                    }
                }
            }
        }
  ...
  for(; d < kernel_depth; d++)
    {
        for(int y = top_left_y; y < y_e; y += dilation_y)
        {
            ...
            {
                // All the values will be the offset (will be zeros when not quantized)
                memset(out_ptr, pad_value, kernel_width * sizeof(int8_t));
            ...
            else
            {
                for(int x = top_left_x; x < x_e; x += dilation_x, ++out_ptr)
                {
                    ...
                    {
                        *out_ptr = *(reinterpret_cast<const int8_t *>(in_ptr + (d * input_stride_z + y * input_stride_y + x * input_stride_x)));
                    }
                }
            }
        }
    ...
    if(has_bias)
    {
        *out_ptr = static_cast<int8_t>(1);
    }
}

} // namespace

NEIm2ColKernel4S8::NEIm2ColKernel4S8()
...
void NEIm2ColKernel4S8::configure(const ITensor *input, ITensor *output, const Size2D &kernel_dims, const PadStrideInfo &conv_info, bool has_bias, const Size2D &dilation, unsigned int num_groups)
{
    ...
    _dilation       = dilation;
    _convolved_dims = scaled_dimensions(input->info()->dimension(width_idx), input->info()->dimension(height_idx), _kernel_width, _kernel_height, _conv_info, _dilation);
    _has_bias = has_bias;
...
Status NEIm2ColKernel4S8::validate(const ITensorInfo *input, const ITensorInfo *output, const Size2D &kernel_dims, const PadStrideInfo &conv_info,
                                bool has_bias, const Size2D &dilation, unsigned int num_groups)
{
...
void NEIm2ColKernel4S8::run(const Window &window, const ThreadInfo &info)
{
    ...
    execute_window_loop(window, [&](const Coordinates & id)
    {
        ...
        auto                 output_ptr = reinterpret_cast<int8_t *>(out.ptr() + (id[width_idx] + id[height_idx] * _convolved_dims.first) * _output->info()->strides_in_bytes().y());
